...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <div className="group relative bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-100 hover:border-transparent hover:-translate-y-2">
      ...
      <div className={`bg-linear-to-br ${bgGradients[product.id as keyof typeof bgGradients]} p-6 pb-16`}>
        <div className="flex items-center justify-between mb-4">
          <div className={`bg-linear-to-br ${gradients[product.id as keyof typeof gradients]} p-3 rounded-xl shadow-lg`}>
            <Icon className="w-6 h-6 text-white" />
          </div>
          {product.id === 3 && (
            <span className="bg-linear-to-br from-amber-500 to-orange-500 text-white text-xs font-bold px-3 py-1 rounded-full">
              POPULAR
            </span>
          )}
        </div>
      ...
      <div className="p-6 -mt-8 relative">
        ...
        <button
          ...
          className={`w-full bg-linear-to-br ${gradients[product.id as keyof typeof gradients]} text-white font-semibold px-6 py-3 rounded-xl hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 group-hover:scale-105`}
        >
